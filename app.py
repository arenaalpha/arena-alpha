...
from modules.comissoes_professor import ComissoesProfessor
...
def organizar_experimentais(experimentais):
    ...
    for aula in experimentais:
        if not aula.get("confirmacao_enviada") or aula.get("resultado"):
            continue
...
def consultar_painel_local():
    if banco_online():
        with conectar() as banco:
            ...
            experimentais = banco.execute("SELECT id, nome, telefone, esporte, data, horario, turma, confirmacao_enviada, resultado, resultado_em FROM aulas_experimentais ORDER BY data, horario, id").fetchall()
...
def enviar_acao_admin(acao, dados):
    if banco_online():
        ...
        if acao == "pagar_comissao_professor":
            resultado = ComissoesProfessor().confirmar_pagamento(int(dados["professor_id"]))
            return {"mensagem": "Comissão de {} paga: R$ {:.2f}. Lançada nas despesas.".format(resultado["nome"], resultado["total"])}
...
def painel_professor():
    ...
    tem_comissao_futvolei = any("fut" in chave_dia_semana(turma["modalidade"]) for turma in turmas) and ComissoesProfessor().professor_recebe_comissao(professor_id)
